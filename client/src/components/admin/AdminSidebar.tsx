      ...
      <ul className="space-y-2">
        {navItems.map(item => {
          ...
          return <li key={item.path}>
            <Link to={item.path} className={`flex items-center space-x-3 px-4 py-3 rounded-sm transition-all duration-300 ${isActive ? 'bg-gold/20 text-gold border-l-2 border-gold font-semibold' : 'text-gray-400 hover:text-white hover:bg-dark/50'}`}>
              <Icon size={18} />
