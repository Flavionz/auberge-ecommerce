...
export const BoutiquePage = () => {
  ...
  const getCategoryClass = (categoryId: string | number) => {
    ...
    return `block w-full text-left py-2 px-3 rounded-sm transition-all duration-300 font-sans 
            ${isActive
        ? 'bg-[#Cca43b]/20 text-[#Cca43b] border-l-2 border-[#Cca43b]' // Attivo
        : 'text-gray-400 hover:text-white hover:bg-[#1E1B18]' // Inattivo
    }`
  }
  ...
  return (
      <div className="min-h-screen bg-[#1E1B18] text-white">
        ...
        <div className="pt-24 pb-20">
          {/* Usiamo un container che simula la classe container-custom (max-w-7xl mx-auto p-4/8) */}
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {isLoading ? (
                <div className="text-center text-xl animate-pulse text-[#Cca43b] h-64">Chargement des délices...</div>
            ) : (
                    ...
                    {filteredProducts.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          {filteredProducts.map((product) => (
                              // Card Style (basato sulla card precedente)
                              <div
                                  key={product.id}
                              ...
                              >
                                {/* Immagine con altezza fissa per coerenza */}
                                <div className="h-48 overflow-hidden relative">
                                  {/* Etichetta Categoria in alto a sinistra */}
                                ...
                                {/* Contenuto Card */}
                                <div className="p-4">
                                  <h3 className="text-lg font-bold mb-1 font-serif text-white">{product.name}</h3>
                              ...
                              {product.price.toFixed(2)} €
                                  ...
                                  <button className="w-full py-2 border border-[#Cca43b] text-[#Cca43b] hover:bg-[#Cca43b] hover:text-black transition-colors rounded uppercase text-xs tracking-wider font-bold">
                                    Ajouter au Panier
                                  </button>
                                </div>
                              </div>
